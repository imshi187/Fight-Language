...
from interpreter.ClassNodes import ClassDeclarationNode, NewObjectNode, MethodCallNode, GetMemberNode, \
    CallClassInnerMethod, ThisNode, InterfaceNode, SerialMethodCallNode, SuperNode, \
    ImportThirdPartyClassNode, ImportBuiltinClassNode, UseModuleNode, ExportsNode, ClassPropertyAssignNode
from interpreter.Nodes import (AssignmentNode, FunctionCallNode, BinaryOpNode,
                               ...
                               StructAssignNode, StructAccessNode, EnumDeclarationNode, EnumAccessNode, DoWhileNode,
                               ListGeneratorNode, AbsNode, SerialCall, PlusAssignNode, MinusAssignNode,
                               MultiplyAssignNode, DivideAssignNode, ObjectAssignNode, ListAssignNode,
                               ImportThirdPartyModuleNode, LogicNotNode, ReAssignmentNode, )
...
class Parser:
    ...
    def parse(self, from_module=None):
        return self.program()
    ...
    def class_property_assign_statement(self):
        ...
        return ClassPropertyAssignNode(class_name, property_name, property_value)

    # 尝试判断是不是在对对象属性进行赋值
    def object_assign_statement(self):
        """
            d{"key"} = val;
        """
        try:
            obj_name = self.current_token_value()
            self.eat_current_token_type("ID")
            self.eat_current_token_type("LBRACE")  # {

            key = self.expr()  # 解析key,一般是字符串，可以是变量

            self.eat_current_token_type("RBRACE")
            self.eat_current_token_type("ASSIGN")
            val = self.expr()
            self.eat_current_token_type("END")
            return ObjectAssignNode(obj_name, key, val)
        except:
            return False

    def list_assign_statement(self):
        """
            d[index] = val;
        """
        try:
            lst_name = self.current_token_value()
            self.eat_current_token_type("ID")
            self.eat_current_token_type("LBRACKET")  # [

            index = self.expr()  # 解析index,一般是数字，可以是变量

            self.eat_current_token_type("RBRACKET")
            self.eat_current_token_type("ASSIGN")
            val = self.expr()
            self.eat_current_token_type("END")
            return ListAssignNode(lst_name, index, val)
        except:
            return False
    ...
    def statement(self):
        ...
        # id+=exp;
        elif self.current_token_type() == 'ID' and (self.peek_next_token_type() in ops) and self.tokens[self.pos + 2][
            0] == 'ASSIGN':
        ...
        # d{"key"}
        elif self.current_token_type() == "ID" and self.peek_next_token_type() == "LBRACE":
            # 比如 d{"key"} = "val";
            val = self.object_assign_statement()  # val是节点或者False
            if isinstance(val, ObjectAssignNode):
                return val
        # d[index] = val
        elif self.current_token_type() == "ID" and self.peek_next_token_type() == "LBRACKET":
            # 比如 d[index] = "val";
            val = self.list_assign_statement()  # val是节点或者False
            if isinstance(val, ListAssignNode):
                return val
        # 类属性赋值  A->b = 10;
        # elif self.current_token_type() == "ID" and self.peek_next_token_type() == "MINUS" and self.tokens[self.pos + 2][
        #     0] == "GT":
        #     return self.class_property_assign_statement()
        ...
        # module 引入
            ...
            if self.peek_next_token_type() == "LT":
                return self.import_class_statement()
        ...
        # super{}
        # elif self.current_token_value() == "super":
        #     print("super")
        ...
        # use
        elif self.current_token_value() == "use":
            return self.use_statement()

        # exports
        elif self.current_token_value() == "exports":
            return self.exports_statement()
    ...
    def exports_statement(self):
        ...
        """
        self.eat_current_token_type("KEYWORD")  # exports
        self.eat_current_token_type("ASSIGN")  # =
        self.eat_current_token_type("LBRACKET")  # [
        exports = []
        while self.current_token_type() != "RBRACKET":
            export_name = self.current_token_value()

            # 模块导出的两种可能
            if self.current_token_type() == "STRING":
                self.eat_current_token_type("STRING")  # "模块1"
    ...
    def use_statement(self):
        ...
        self.eat_current_token_type("KEYWORD")  # use
        ...
        # 第三方模块
        path = self.current_token_value()
        if self.current_token_type() == "STRING":
            self.eat_current_token_type("STRING")  # "路径"
            return UseModuleNode(import_names, path)

        # 内置模块
        elif self.current_token_type() == "ID":
            self.eat_current_token_type("ID")  # 内置模块
            return UseModuleNode(import_names, path, is_builtin=True)

    def import_class_statement(self):
        """
            import <类名称> from "路径"
        """
        ...
        if self.current_token_type() == "KEYWORD":
            ...
            path = self.current_token_value()
            self.eat_current_token_type("STRING")  # "路径"
            return ImportThirdPartyClassNode(class_name, path)
        else:
            # 导入内置的类
            return ImportBuiltinClassNode(class_name)

    def super_statement(self):
        """
            super{ key:val, key2:val2 }
        """
        self.eat_current_token_type("KEYWORD")  # super
        self.eat_current_token_type("LBRACE")  # {
        properties = {}
    ...
    def variable_declaration(self):
        ...
        self.eat_current_token_type('COLON')  # :
        # 数据类型
        type_name = self.current_token_value()  # type_name是str类型

        if self.current_token_type() == "ID":
            self.eat_current_token_type("ID")  # 类型名,比如结构体、枚举
        if self.current_token_type() == "KEYWORD":  # 内置类型
            self.eat_current_token_type("KEYWORD")
        # ================= 类型check======================
        ...
        if self.current_token_type() == "END":
            self.eat_current_token_type("END")  # ;
        return AssignmentNode(var_name, var_value, data_type=type_name)
    ...
    def struct_statement(self):
        ...
        while self.current_token_type() != "RBRACE":
            ...
            # ==================== 检查类型===============
            self.eat_current_token_type("COLON")  # :
            property_type = self.current_token_value()
            # self.eat_current_token_type("KEYWORD")  # 类型
            self.eat_data_type()

            # 记录类型
            properties_types.update({property_name: property_type})
            # ==================== 检查类型===============
        ...
        self.eat_current_token_type("RBRACE")  # }
        return StructDeclarationNode(struct_name, properties, properties_types)
    ...
    def interface_statement(self):
        ...
        while self.current_token_value() != "}":
            # 解析方法的返回类型
            return_type = self.current_token_value()
            self.eat_data_type()
            ...
            args = []
            while self.current_token_type() != 'RPAREN':
                params_types.append(self.current_token_value())
                self.eat_data_type()
            ...
            if self.current_token_type() == "END":
                self.eat_current_token_type("END")  # :
        self.eat_current_token_type("RBRACE")  # }
        return InterfaceNode(interface_name, methods, params_types=params_types, return_type=return_type)

    def eat_data_type(self):
        """
            解析数据类型
        """
        if self.current_token_type() == "KEYWORD":
            self.eat_current_token_type("KEYWORD")  # 关键字类型
        elif self.current_token_type() == "ID":
            self.eat_current_token_type("ID")  # set类型、自定义类型
    ...
    def evaluate_call_class_inner_method(self):
        ...
        while self.current_token_type() != 'RPAREN':
            ...
            if self.current_token_type() == 'ID' and self.peek_next_token_type() == 'ASSIGN':  # 解析命名参数
                id_name = self.current_token_value()  # 解析参数名
                self.eat_current_token_type('ID')  # 解析参数名
                self.eat_current_token_type('ASSIGN')  # =
                named_args[id_name] = self.expr()  # 解析参数值
            else:
                args.append(self.expr())
            if self.current_token_type() == 'COMMA':
                self.eat_current_token_type('COMMA')
        ...
        return CallClassInnerMethod(method_name, args, named_args=named_args)
    ...
    def method_call(self):
        ...
        while self.current_token_type() != 'RPAREN':
            ...
            # 解析命名参数
            if self.current_token_type() == 'ID' and self.peek_next_token_type() == "ASSIGN":
                ID = self.current_token_value()
                self.eat_current_token_type('ID')
                self.eat_current_token_type('ASSIGN')
                named_args[ID] = self.expr()
            else:
                # 解析位置参数
                args.append(self.expr())
        ...
        # ===========================连续调用============================
        methods = []
        methods.append(MethodCallNode(instance_name, method_name, args, named_args=named_args))

        # 实例进行连续调用
        while self.current_token_type() == "MINUS" and self.peek_next_token_type() == "GT":
            method_node = self.single_method_call(instance_name)
            methods.append(method_node)

        if len(methods) > 1:
            return SerialMethodCallNode(instance_name, methods)
        # ===========================连续调用============================

        return MethodCallNode(instance_name, method_name, args, named_args=named_args)

    def single_method_call(self, instance_name=None):
        """
            解析实例调用方法的表达式 比如 p->show();
    ...
    def class_declaration(self):
        # 进来的时候是class关键字
        global init_method, fields_annotations, init_methods
        ...
        while self.current_token_value() != "}":
            ...
            if self.current_token_value() == "fields":
                ...
                while self.current_token_type() == "ID" or self.current_token_value() == "static" or self.current_token_value() == "@":
                    ...
                    # Name: int = 1;
                    self.eat_current_token_type("COLON")
                    field_type = self.current_token_value()
                    fields_types[field] = field_type  # 存入字段的类型
                    print("fields_types: ", fields_types)

                    if self.current_token_type() == "KEYWORD":
                        self.eat_current_token_type("KEYWORD")  # int
                    else:
                        self.eat_current_token_type("ID")  # set被解析为ID类型
                    # ================增加类型====================
            ...
            if self.current_token_value() == "methods":
                ...
                while self.current_token_value() == "def" or self.current_token_value() == "static" or self.current_token_value() == "@":
                    ...
                    print("解析得到的方法node: ", method_def_node)
            ...
            # ===========尝试解析多个构造方法==============================
            init_methods = []
            while self.current_token_value() == "init":
                if_init_parsed = True
                init_method = self.parse_init_method()
            ...
            # print("\nlen(init_methods): ", len(init_methods), "\n")
    ...
    def parse_init_method(self):
        ...
        while self.current_token_type() != 'RPAREN':
            # 解析参数
            param_name = self.current_token_value()  # 参数名
            ...
            field_type = self.current_token_value()
            if self.current_token_type() == "KEYWORD":
                self.eat_current_token_type("KEYWORD")  # int float等类型
            else:
                self.eat_current_token_type("ID")  # set类型被解析为ID类型
            params_types[param_name] = field_type  # 存入参数的类型
            # ============类型检查=======================
        ...
        # super {}
        while self.current_token_type() != 'RBRACE':
            if self.current_token_value() == "super":
                super_node = self.super_statement()
                body.append(super_node)
            else:
                stmt = self.statement()
                body.append(stmt)
        return FunctionDeclarationNode("init", params, body, default_values, params_types=params_types)
    ...
    # 引入模块机制
    # 区别导入内部模块和第三方模块：
    # 内部模块：import 后面或者from后面是ID
    # 第三方模块：import 后面或者from后面是字符串
    def import_module_statement(self):
        ...
        if self.current_token_type() == "LPAREN":  # (
            ...
            self.eat_current_token_type("STRING")  # 模块路径
            return ImportModuleNode(module_name, import_elements, import_whole_module=False, third_party=True)
        ...
        if self.current_token_type() == "LBRACE":
            ...
            # 第三方模块 import {模块名称} from "模块路径"
            elif self.current_token_type() == "STRING":
                self.eat_current_token_type("STRING")
                print("import elements: ", import_elements)
                return ImportThirdPartyModuleNode(module_name, import_elements[0])
    ...
    def factor(self):
        ...
        elif self.current_token_type() == "BANG":  # !
    ...
    def logic_not(self):
        # !true
        self.eat_current_token_type("BANG")  # !
        expr = self.expr()
        # print("entered logic_not", expr)
        print("entered logic_not： ", LogicNotNode(expr))
        return LogicNotNode(expr)
    ...
    def struct_access_expr(self):
        ...
        while self.current_token_type() == 'ID':
            ...
            fields_names.append(struct_field_name)
        ...
        if len(fields_names) == 1:
            ...
            return StructAccessNode(struct_instance_name, None,fields_names)
    ...
    def evaluate_new_expr(self):
        # let z: type = new 类名();
        object_name = self.tokens[self.pos - 4][1]
    ...
    def arrow_function_declaration(self):
        ...
        while self.current_token_type() != 'GT':
            # 解析参数
            param_name = self.current_token_value()
            params.append(param_name)  # 参数名称
            self.eat_current_token_type('ID')

            # ============= 解析参数========================
            self.eat_current_token_type('COLON')  # :
            param_type = self.current_token_value()
            if self.current_token_type() == 'ID':
                self.eat_current_token_type('ID')  # 类型名(set类型被解析为ID)
            else:
                self.eat_current_token_type('KEYWORD')  # 类型名
            params_types[param_name] = param_type
            # ============= 解析参数========================
        ...
        self.eat_current_token_type('RBRACE')  # }
        # ; 不需要处理
        return FunctionDeclarationNode(arrow_func_name, params, body, default_values, return_type=return_type,
    ...
    def anonymous_function_declaration(self, passin_name=None):
        ...
        return FunctionDeclarationNode(lambda_name, params, body, default_values, func_type="anonymous",
                                       return_type=return_type, params_types=params_types)
    ...
    def class_method_declaration(self):
        ...
        return FunctionDeclarationNode(name, params, body, default_values, is_static, tag="class_method",
                                       annotations=method_annotation, params_types=params_types,
    ...
    def function_declaration(self):
        ...
        while self.current_token_type() != 'RPAREN':
            ...
            param_type = self.current_token_value()
            if self.current_token_type() == 'ID':
                self.eat_current_token_type('ID')  # 类型名(set类型被解析未id)
            else:
                self.eat_current_token_type('KEYWORD')  # 类型名
            params_types[param_name] = param_type
            print("params_return_type: ", params_types)
            # =========== 解析参数类型=================
        ...
        return FunctionDeclarationNode(name, params, body, default_values, params_types=params_types,
                                       return_type=return_type)
    ...
    def let_statement(self):
        ...
        # if type_name == "set":  # set被看作关键字了
        #     self.eat_current_token_type("KEYWORD")
        # else:
        #     if self.current_token_type() != 'DATA_TYPE':
        #         raise Exception("data type error")
        #     self.eat_current_token_type("DATA_TYPE")  # 数据类型
        # ================= 类型check======================
        ...
        if self.current_token_type() == 'END':
            self.eat_current_token_type('END')
        return AssignmentNode(name, value, data_type=type_name)
    ...
    def parse_single_function_call(self, func_expr=None):
        ...
        while self.current_token_type() != 'RPAREN':  # 解析第一个参数
            # 函数参数可以是任意表达式，比如1+2, add(), true and true, 1>2, "hello"等
            if self.current_token_type() == 'ID' and self.peek_next_token_type() == 'ASSIGN':  # 解析命名参数
                id = self.current_token_value()  # 解析参数名
                self.eat_current_token_type('ID')  # 解析参数名
                self.eat_current_token_type('ASSIGN')  # =
                named_args[id] = self.expr()  # 解析参数值
            else:
                args.append(self.expr())
            if self.current_token_type() == 'COMMA':  # 解析多个参数
                self.eat_current_token_type('COMMA')  # ,
        self.eat_current_token_type('RPAREN')  # )
        ...
        return FunctionCallNode(name, args, named_args)
    ...
    def eat_current_token_type(self, token_type):
        ...
        if self.tokens[self.pos][0] == token_type:
            ...
            raise SyntaxError(f"Expected {token_type} but got {self.tokens[self.pos][0]}={self.tokens[self.pos][1]}")
