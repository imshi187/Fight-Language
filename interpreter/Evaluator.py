...
from interpreter.ClassNodes import ClassDeclarationNode, NewObjectNode, MethodCallNode, GetMemberNode, \
    ...
    ImportThirdPartyClassNode, ImportBuiltinClassNode, UseModuleNode
...
from interpreter.Nodes import AssignmentNode, NumberNode, BinaryOpNode, VariableNode, FunctionCallNode, StringNode, \
    ...
    ImportThirdPartyModuleNode, UnaryOpNode
...
class Evaluator:
    ...
    def evaluate_function_call3(self, node: FunctionCallNode):
        """
            核心想法：
                模块环境合并main env, 如果同名，则模块环境的内容覆盖
                main env的内容

            说明：
                evaluate_function_call就是在evaluate_function_call2的基础上上进行包装，
                主要是进行环境的合并和恢复

        """

        is_from_module = self.check_is_from_module(node.name)
        print("is_from_module: ", is_from_module)

        # 保存旧环境
        old_env = self.environment.copy()

        print("\n\n(eval_fn_call)模块used elements: ", self.used_elements)
        # 问题：需要判断是不是在use (xx) from xx 模块！！！！！
        # print("模块： ",is_from_module," 注意，我要调用的函数是： ",node.name,node.name in self.used_elements[is_from_module], "\n\n")

        # 说明是来自模块的函数
        if isinstance(is_from_module, str) and node.name in self.used_elements[is_from_module]:
            # 导出的 & 被use的，可以使用模块内部的内容
            # 找到模块环境
            module_env = self.modules[is_from_module]
            print("模块环境： ", module_env)

            # 合并模块内部的和外部的环境（核心）
            self.environment.update(module_env)

            # 使用合并后的环境进行函数调用
            value = self.evaluate_function_call(node)
        ...
        # 说明是来自main 环境的函数
            ...
            print("\n本轮调用的函数是： ", node.name)
            value = self.evaluate_function_call(node)
            print("本轮得到的结果value: ", value, "\n")
            return value

    def get_module_env(self, final_path):
        # 第三方模块
        # 1,读出代码
        # module_path = node.module_path
        f = open(final_path, "r", encoding="utf-8")
        code = f.read()
        f.close()
        # print("code:\n ",code)

        # 2. 全部加载到环境中加载代码
        pkg_tokenizer = Tokenizer(code)
        ...
        # 将模块里面没有导出的变量，函数等，导入到一个独立的模块环境中，
        # 如果被导出的函数之类的需要访问，可以通过这个环境得到变量、函数等
        # =================================================================
        for import_node in pkg_ast[:-1]:
            self.evaluate(import_node, env=module_env)  # 问题在这里,ClassDeclarationNode的解析方法的问题
        # =================================================================

        # 这个时候self.environment是module_env

        # 恢复环境
        self.environment = old_env  # Test还存在
        print("模块环境： ", module_env)
        return module_env
    ...
    def evaluate_class_declaration(self, node: ClassDeclarationNode):
        ...
        # print("mynode: ", node.interfaces)i
    ...
    def evaluate_if_statement(self, node):
        ...
        if condition_value:  # 如果条件为真，执行 if 代码块
            ...
            for statement in node.if_body:
                print("statement(if_statement): ", statement)
...
if __name__ == '__main__':
    # ================================================================
    code = """
           use (  foreach ) from ListUtils
          
           foreach([1,2,3],def(item){
                printlnCyan(item);
           });
